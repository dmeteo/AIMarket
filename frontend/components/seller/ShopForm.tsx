...
interface ShopFormProps {
  ...
  onSave: (data: { name: string; description: string; logo_url?: string }) => Promise<void>;
  onCancel: () => void;
  maxShops?: number;
  currentCount?: number;
}

export default function ShopForm({ shop, onSave, onCancel, maxShops = 10, currentCount = 0 }: ShopFormProps) {
  ...
  if (!canCreate) {
    return (
      <div className="text-center py-8">
        <p className="text-gray-500 mb-4">Достигнут лимит магазинов ({maxShops}). Удалите ненужные магазины для создания новых.</p>
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <div className="flex items-center gap-4">
          ...
          <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/gif" onChange={handleLogoChange} className="hidden" />
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        <div className="flex gap-3">
          ...
          <Button variant="primary" onClick={handleSubmit} disabled={saving || !name.trim()}>
            {saving ? 'Сохранение...' : isEditing ? 'Сохранить' : 'Создать магазин'}
          </Button>
        </div>
      </div>
    </div>
